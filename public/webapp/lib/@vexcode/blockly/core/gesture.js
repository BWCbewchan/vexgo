...
Blockly.Gesture.prototype.updateIsDraggingBlock_ = function() {
  ...
  if (this.flyout_) {
    ...
    isFlyoutStart = true;
    // RM Code End
  } else if (this.targetBlock_.isMovable() || this.shouldDuplicateOnDrag_) { // RM added duplicate code from scratch
    this.isDraggingBlock_ = true;
  }

  if (this.isDraggingBlock_) {
    // RM Code Start
    this.startDraggingBlock_(isFlyoutStart);
    // RM Code End
    return true;
  }
  return false;
 ...
 * @param {boolean?} toolboxStart indicates if the drag started from the toolbox
 * @private
 */
Blockly.Gesture.prototype.startDraggingBlock_ = function(toolboxStart) {
  // RM code start
  // from scratch to allow dupliacte block on drag
  ...
  // RM Code Start
  if (toolboxStart) {
    this.blockDragger_.dragFromToolbox = true;
  }
  // RM Code End
};
...
Blockly.Gesture.prototype.setStartBlock = function(block) {
  ...
  if (!this.startBlock_ && !this.startBubble_) {
    ...
    // RM code start
    // code from scratch to handle dupliacting blocks on drag like myblock arg blocks
    this.shouldDuplicateOnDrag_ =
      Blockly.utils.scratch.isShadowArgumentReporter(block);
    // RM code end
...
Blockly.Gesture.prototype.setTargetBlock_ = function(block) {
  if (block.isShadow() && !this.shouldDuplicateOnDrag_) { // RM - added duplicate on drag code
