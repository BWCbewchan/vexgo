...
Blockly.InsertionMarkerManager = function(block) {
  ...
  // RM Code Start
  /**
   * Whether the block would be monitored if it were dropped immediately.
   * Updated on every mouse move.
   * @type {boolean}
   * @private
   */
 ...
 * @package
 */
Blockly.InsertionMarkerManager.prototype.wouldMonitorBlock = function() {
  return this.wouldMonitorBlock_;
};
 ...
 * @package
 */
Blockly.InsertionMarkerManager.prototype.update = function(dxy, deleteArea, monitorArea) {
  var candidate = this.getCandidate_(dxy);

  this.wouldDeleteBlock_ = this.shouldDelete_(candidate, deleteArea);
  this.wouldMonitorBlock_ = this.shouldMonitor_(candidate, monitorArea);
...
// RM Code Start
/**
 * Whether ending the drag would monitor the block.
 * @param {!Object} candidate An object containing a local connection, a closest
 *     connection, and a radius.
 * @param {?number} monitorArea One of {@link Blockly.MONITOR_AREA_NONE},
 *     or {@link Blockly.MONITOR_AREA_DROP}.
 * @return {boolean} True if dropping the block immediately would trigger
 *     a monitor event for the block.  False otherwise.
 * @private
 */
Blockly.InsertionMarkerManager.prototype.shouldMonitor_ = function(candidate,
    monitorArea) {
  // Prefer connecting over dropping into the trash can, but prefer dragging to
  // the toolbox over connecting to other blocks.
  var wouldConnect = candidate && !!candidate.closest;
  var wouldMonitor = !!monitorArea;

  return wouldMonitor && !wouldConnect;
};
// RM Code End
...
Blockly.InsertionMarkerManager.prototype.maybeHidePreview_ = function(candidate) {
  ...
  if (!candidate.closest) {
    ...
    // RM Code Start
    if (hadPreview &&
      (closestChanged || localChanged || this.wouldDeleteBlock_ || this.wouldMonitorBlock_)
    ) {
      // RM Code End
      this.hidePreview_();
    }
