...
Blockly.Workspace.prototype.undo = function(redo) {
  ...
  if (window && window.dispatchEvent) {
    window.dispatchEvent(new CustomEvent("research", {detail: {action: redo ?  "redo" : "undo"}}))
  }
  // RM Code End
  ...
  // RM Code Start
  // Ticket: https://robomatter.atlassian.net/browse/VC-8617
  var RM_shouldRefreshToolbox = false;
  // RM Code End
  if (!inputEvent) {
    return;
  }
  ...
  try {
    for (var i = 0, event; (event = events[i]); i++) {
      // RM Code Start
      // If in unde/redo event there is a "myblock" then flip the flag true to refresh
      if (event && event.blockId) {
        var RM_Temp_Myblock = this.getBlockById(event.blockId);
        if (RM_Temp_Myblock && RM_Temp_Myblock.type && RM_Temp_Myblock.type === "procedures_definition") {
          // Looks like undo/redo is doing something with my blocks, refresh the toolbox
          RM_shouldRefreshToolbox = true;
        }
      }
      // RM Code End
      event.run(redo);
    ...
    // RM Code Start
    // Refresh toolbox is flipped, we need to refresh the toolbox
    if (RM_shouldRefreshToolbox) {
      // TODO: This might be issue when we have multiple workspaces.
      // https://git.innovationfirst.net/vexcode/vexcode-blockly/-/merge_requests/56#note_156875
      Blockly.mainWorkspace.refreshToolboxSelection();
    }
    // RM Code End
  }
};
