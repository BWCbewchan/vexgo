...
Blockly.Toolbox.prototype.init = function() {
  ...
  themeManager.subscribe(this.HtmlDiv, 'toolboxCategoryBackgroundColour',
      'background-color');
  themeManager.subscribe(this.HtmlDiv, 'toolboxCategoryForegroundColour', 'color');
};
...
Blockly.Toolbox.prototype.refreshSelection = function() {
  // RM Code Start
  this.flyout_.show(this.getContents_());
  this.flyout_.updateCategorySelectionByScrollPosition();
 ...
 * @protectd
 */
Blockly.Toolbox.prototype.getContents_ = function() {
  const content = [];
  for (var i = 0; i < this.contents_.length; i++) {
    const subItem = this.contents_[i];
    if (subItem.getLabelItem) {
      content.push(subItem.getLabelItem());
    }
    const subContent = subItem.getContents();
    if (subContent) {
...
Blockly.Toolbox.prototype.setSelectedItem = function(newItem) {
  ...
  if (this.shouldDeselectItem_(oldItem, newItem) && oldItem != null) {
    // RM Code Start
    if (newItem == oldItem && this.flyout_.autoClose) {
...
Blockly.Toolbox.prototype.updateFlyout_ = function(oldItem, newItem) {
  ...
  if (newItem) {
    this.flyout_.show(this.getContents_());
    this.setSelectedCategoryById(newItem.id_);
  } else {
    this.flyout_.hide();
  }
