...
// RM Code Start
goog.requireType('Blockly.IToolboxCategoryColour');
// RM Code End
...
Blockly.ToolboxCategory.prototype.createIconDom_ = function() {
  var toolboxIcon = document.createElement('div'); // RM changed from span to div
  if (!this.parentToolbox_.isHorizontal()) {
    Blockly.utils.dom.addClass(toolboxIcon, this.cssConfig_['icon']);
  }

  // RM Code change - we want block not inline-block
...
Blockly.ToolboxCategory.prototype.createLabelDom_ = function(name) {
  var toolboxLabel = document.createElement('div'); // RM changed from span to div
...
Blockly.ToolboxCategory.prototype.addColourBorder_ = function(colour) {
  if (colour) {
    // RM Code Start
    // changed to only set the color
    const backgroundColor = colour.colourPrimary || '#ddd';
    const borderColor = colour.colourSecondary || '#ddd';
    this.iconDom_.style.backgroundColor = backgroundColor;
    this.iconDom_.style.borderColor = borderColor;
    // RM Code End
  }
};
...
Blockly.ToolboxCategory.prototype.getColour_ = function(categoryDef) {
  ...
  const colour2 = categoryDef['secondaryColour'];
  ...
  if (colour && styleName) {
    ...
    // RM Code Start
    const colourPrimary = this.parseColour_(colour);
    const colourSecondary = colour2 ? this.parseColour_(colour2) : colourPrimary;
    return {colourPrimary, colourSecondary};
    // RM Code End
  }
  // RM change return type
  return {colourPrimary: '', colourSecondary: ''};
};
...
Blockly.ToolboxCategory.prototype.getColourfromStyle_ = function(styleName) {
  ...
  if (styleName && theme) {
    ...
    if (style && style.colour) {
      // RM Code Start
      const colourPrimary = this.parseColour_(style.colour);
      const colourSecondary = style.colourSecondary ? this.parseColour_(style.colourSecondary) : colourPrimary;
      return {colourPrimary, colourSecondary};
      // RM Code End
    } else {
 ...
 * @public
 */
Blockly.ToolboxCategory.prototype.getLabelItem = function() {
  return [{
    "kind": "label",
    "text": this.name_,
    "web-class": "categoryLabel",
    "category-label": true, // not sure if this will do anything
    "id": this.id_,
  }];
};
// RM Code End
