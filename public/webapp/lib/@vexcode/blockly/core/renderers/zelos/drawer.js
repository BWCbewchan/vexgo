...
Blockly.zelos.Drawer.prototype.drawRightSideRow_ = function(row) {
  ...
  // RM Code Start
  const isDefine = this.block_.hat === "define";
  if (!isDefine && (row.precedesStatement || row.followsStatement)) {
  //RM Code End
...
Blockly.zelos.Drawer.prototype.drawStatementInput_ = function(row) {
  // RM Code Start
  // if this is a my block definition block, we do not want to draw the input area
