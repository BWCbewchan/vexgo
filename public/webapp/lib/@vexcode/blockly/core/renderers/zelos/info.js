...
Blockly.zelos.RenderInfo.prototype.getInRowSpacing_ = function(prev, next) {
  ...
  if (!prev) {
    // RM Code Start
    // for blocks that start with text we want to increas the padding to align with mixed mode blocks
    if (next && Blockly.blockRendering.Types.isField(next) && next.field instanceof Blockly.FieldLabel) {
...
Blockly.zelos.RenderInfo.prototype.getElemCenterline_ = function(row, elem) {
  // RM Code Start
  // if we need to adjust the block height to avoid the notch, we need to
  // shift down the element.
  // Offset of notch height is divided by 10 as opposed to 12 as we changed the offset.
  var offset = row && row.rowNeedsNotchShift ? this.constants_.NOTCH_HEIGHT / 10 : 0;
  // RM Code End
  if (row.hasStatement && !Blockly.blockRendering.Types.isSpacer(elem) &&
      !Blockly.blockRendering.Types.isStatementInput(elem)) {
    return row.yPos + this.constants_.EMPTY_STATEMENT_INPUT_HEIGHT / 2 + offset; // RM change - add offset for notch avoidance
  }
  if (Blockly.blockRendering.Types.isInlineInput(elem)) {
    ...
    if (connectedBlock && connectedBlock.outputConnection &&
      ...
      return row.yPos + connectedBlock.height / 2 + offset; // RM change - add offset for notch avoidance
    }
  }
  return Blockly.zelos.RenderInfo.superClass_.getElemCenterline_.call(this,
      row, elem) + offset; // RM change - add offset for notch avoidance
};
      ...
      for (var j = 0, elem; (elem = row.elements[j]); j++) {
        ...
        if (prevInRowSpacer && (Blockly.blockRendering.Types.isField(elem) ||
            Blockly.blockRendering.Types.isInput(elem))) {
              // RM Code Start
              // Setting an offset for when to actually apply a notch shift
              // So that blocks with a field or input close to the notch but isn't really under it doesn't trigger notch shift
              minXPos -= 5;
              // RM Code End
          if (xCursor < minXPos &&
            ...
            // RM Code Start
            // Assign rowNeedsNotchShift to trigger resize
            row.rowNeedsNotchShift = true;
            // RM Code End
          }
        }
        xCursor += elem.width;
 ...
 * @protected
 */
Blockly.zelos.RenderInfo.prototype.addRowSpacing_ = function() {
  if (this.block_.hat !== 'define') {
    Blockly.zelos.RenderInfo.superClass_.addRowSpacing_.call(this);
    return;
  }
  
  // we need to handle define block a little different
  ...
  for (var r = 0; r < oldRows.length; r++) {
    ...
    if (r != oldRows.length - 1) {
      const nextRow = oldRows[r + 1];
      // we do not want to add a spacer row before the bottom row
      if (!Blockly.blockRendering.Types.isBottomRow(nextRow)) {
        this.rows.push(this.makeSpacerRow_(oldRows[r], nextRow));
      }
    }
  }
};
...
Blockly.zelos.RenderInfo.prototype.computeBounds_ = function() {
  ...
  // Adjust the width for the my block definition block
  ...
  for (let i = 0, row; (row = this.rows[i]); i++) {
    ...
    let statementInputElemWidth = 0;
    for (let elemIndex = 0; elemIndex < row.elements.length; elemIndex++) {
      const element = row.elements[elemIndex];
      if (!Blockly.blockRendering.Types.isStatementInput(element)) {
        continue;
      }
      statementInputElemWidth = element.width;
    }
    const rowWidth = row.widthWithConnectedBlocks - statementInputElemWidth;
    blockWidth = Math.max(blockWidth, rowWidth);
  }

  this.width = blockWidth + this.constants_.LARGE_PADDING;
};
// RM Code End