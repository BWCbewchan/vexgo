...
Blockly.Variables.createVariableButtonHandler = function(
  ...
  // RM - pulled from scratch and modified
  ...
  // TODO: let VEXcode handle the title?
  ...
  } else if (type == Blockly.ARRAY2D_VARIABLE_TYPE) {
    newMsg = Blockly.Msg.NEW_ARRAY_TITLE;
    modalTitle = Blockly.Msg.ARRAY_MODAL_TITLE;
  } else if (type == Blockly.BOOL_VARIABLE_TYPE) {
    newMsg = Blockly.Msg.NEW_BOOL_TITLE;
    modalTitle = Blockly.Msg.BOOL_MODAL_TITLE;
  } else {
    // for scalar and all other variable types
        ...
        function(text, varOptions) {
          if (text) {
            ...
            if (existing) {
              ...
              // RM added
              const arrayLength = varOptions && varOptions.arrayLength ? varOptions.arrayLength : 0;
              const arrayWidth = varOptions && varOptions.arrayWidth ? varOptions.arrayWidth : 0;
              // RM end

              // No conflict
              workspace.createVariable(text, type, undefined, arrayLength, arrayWidth);
              if (opt_callback) {
                opt_callback(text);
              }
            }
...
Blockly.Variables.renameVariable = function(workspace, variable,
  ...
  // This function needs to be named so it can be called recursively.
  // RM Code Start
  var RENAME_TITLE = variable.type === 'broadcast_msg' ? 'RENAME_MESSAGE_TITLE' : 'RENAME_VARIABLE_TITLE';
  // RM Code End
  var promptAndCheckWithAlert = function(defaultName) {
    // RM Code Start
    var promptText =
        Blockly.Msg[RENAME_TITLE].replace('%1', variable.name);
    // RM Code End
 ...
 */
Blockly.Variables.promptName = function(promptText, defaultText, callback, opt_title, opt_varType) {
  Blockly.prompt(promptText, defaultText, function(newVar, varOptions) {
    ...
    callback(newVar, varOptions);
  }, opt_title, opt_varType
  );
};
 ...
 */
 Blockly.Variables.generateVariableFieldXml_ = function(variableModel, opt_name) {
  ...
  var text = '<field name="' + fieldName + '" id="' + variableModel.getId() +
    '" variabletype="' + Blockly.utils.xml.escapeHtml(typeString) +
    '">' + Blockly.utils.xml.escapeHtml(variableModel.name) + '</field>';
  return text;//Blockly.utils.xml.domToText(field);
};
...
Blockly.Variables.createVariable_ = function(workspace, id, opt_name,
    opt_type) {
  // TODO: extend this to support list/array dimensions?
  var potentialVariableMap = workspace.getPotentialVariableMap();
 ...
 */
Blockly.Variables.allDeveloperVariablesData = function(workspace) {
  var blocks = workspace.getAllBlocks(false);
  var variableHash = Object.create(null);
  for (var i = 0, block; (block = blocks[i]); i++) {
    var getDeveloperVariables = block.getDeveloperVariables;
    ...
    if (getDeveloperVariables) {
      var devVars = getDeveloperVariables();
      for (var j = 0; j < devVars.length; j++) {
        const devVar = devVars[j];
        if (devVar.name) {
          variableHash[devVar.name] = devVar;
        } else {
          variableHash[devVar] = devVar;
        }
      }
    }
  }
  ...
  // Flatten the hash into a list.
  return Object.values(variableHash);
};
