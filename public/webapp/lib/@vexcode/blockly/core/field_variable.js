...
Blockly.FieldVariable.prototype.initModel = function() {
  // RM Code Start - VC-6726 fix 2d list creating extra list variables (i, j, k .. etc)
...
Blockly.FieldVariable.dropdownCreate = function() {
  ...
  // RM Code Start - If broadcast type, add create variable option
  if (this.defaultType_ == Blockly.BROADCAST_MESSAGE_VARIABLE_TYPE) {
    ...
    options.push([Blockly.Msg["RENAME_MESSAGE"], Blockly.RENAME_VARIABLE_ID])
    if (Blockly.Msg['DELETE_MESSAGE']) {
      options.push(
          [
            Blockly.Msg['DELETE_MESSAGE'].replace('%1', name),
            Blockly.DELETE_BROADCAST_MESSAGE_ID
          ]
      );
    }
  } else {
...
Blockly.FieldVariable.prototype.onItemSelected_ = function(menu, menuItem) {
  ...
  if (this.sourceBlock_ && this.sourceBlock_.workspace) {
    if (id == Blockly.RENAME_VARIABLE_ID) {
      ...
      return;
    // RM Code Start - Create new variable (event message)
    } else if (id == Blockly.NEW_BROADCAST_MESSAGE_ID) {
      Blockly.Variables.createVariable(
          this.sourceBlock_.workspace, null,Blockly.BROADCAST_MESSAGE_VARIABLE_TYPE);
      return;
    } else if (id == Blockly.DELETE_BROADCAST_MESSAGE_ID) {
