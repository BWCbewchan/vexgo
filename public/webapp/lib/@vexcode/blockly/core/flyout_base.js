...
// RM Code Start
goog.requireType('Blockly.IFlyoutCategoryPositionData');
// RM Code End
 ...
 * @type {boolean}
 */
Blockly.Flyout.prototype.autoClose = false; // RM change to default to stay open
...
Blockly.Flyout.prototype.show = function(flyoutDef) {
  ...
  // RM Code Start
  // process everything as an array to keep code simple
  ...
  for (let index = 0; index < flyoutDef.length; index++) {
    if (typeof flyoutDef[index] == 'string') {
      flyoutDef[index] = this.getDynamicCategoryContents_(flyoutDef[index]);
    }
  }
  // RM Code End
  ...
  var parsedContent = Blockly.utils.toolbox.convertFlyoutDefArrayToJsonArray(flyoutDef);
  var flyoutInfo =
 ...
 */
Blockly.Flyout.prototype.categoryScrollPositions = null;
  ...
  for (var i = 0; i < this.buttons_.length; i++) {
    if (this.buttons_[i].getIsCategoryLabel()) {
      ...
      this.categoryScrollPositions.push({
        categoryName: categoryLabel.getButtonText(),
...
Blockly.Flyout.prototype.selectCategoryByScrollPosition = function (pos) {
  ...
  if (workspacePos === 0) {
    this.parentToolbox_.selectCategoryById(this.categoryScrollPositions[0].categoryId);
    return;
  }
...
Blockly.Flyout.prototype.setScrollPos = function (pos) {
  this.scrollbar.set(pos * this.workspace_.scale);
};
...
Blockly.Flyout.prototype.stepScrollAnimation = function() {
  ...
  this.scrollbar.set(scrollPos + diff * this.scrollAnimationFraction);
