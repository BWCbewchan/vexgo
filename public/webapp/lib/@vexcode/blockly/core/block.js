 ...
 * @return {string} Text of block.
 */
Blockly.Block.prototype.toStringCustom = function(opt_maxLength, opt_emptyToken, callback) {
  var text = [];
  var emptyFieldPlaceholder = opt_emptyToken || '?';
  ...
  while (node) {
    switch (node.getType()) {
      case Blockly.ASTNode.types.INPUT:
        ...
        break;
      case Blockly.ASTNode.types.FIELD:
        var special = callback(node); // Returns custom string or undefined from VEXcode
        if (special !== undefined && typeof special === "string") {
          // If VEXcode returns a custom message use that
          text.push(special);
        } else {
          // Otherwise stick to default logic
          var field = /** @type {Blockly.Field} */ (node.getLocation());
          if (field.name != Blockly.Block.COLLAPSED_FIELD_NAME) {
            text.push(field.getText());
          }
