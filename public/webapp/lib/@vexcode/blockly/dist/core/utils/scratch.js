 ...
 */
  ...
  * @namespace
  **/
 goog.provide('Blockly.utils.scratch');
  ...
  * @package
  */
 Blockly.utils.scratch.measureText = function(fontSize, fontFamily,
     fontWeight, text) {
  ...
  * @package
  */
 Blockly.utils.scratch.encodeEntities = function(rawStr) {
  ...
  * @package
  */
 Blockly.utils.scratch.changeObscuredShadowIds = function(block) {
  ...
  * @package
  */
 Blockly.utils.scratch.isShadowArgumentReporter = function(block) {
   return (
     block.isShadow() &&
     (
  ...
  */
 Blockly.utils.scratch.compareStrings = function(str1, str2) {
  ...
  * @package
  */
 Blockly.utils.scratch.blockIsRecyclable = function(block) {
   ...
   for (var i = 0; i < block.inputList.length; i++) {
     ...
     if (input.connection) {
       ...
       if (child && !Blockly.utils.scratch.blockIsRecyclable(child)) {
         return false;
       }
     }
   }
  ...
  * @package
  */
 Blockly.utils.scratch.duplicateAndDragCallback = function(oldBlock, event) {
   ...
   return function(e) {
     ...
     setTimeout(function() {
       ...
       try {
         ...
         Blockly.utils.scratch.changeObscuredShadowIds(newBlock);
  ...
  */
 Blockly.utils.scratch.convertCallback = function(oldBlock) {
   return function() {
     setTimeout(function () {
       try {
         var ws = oldBlock.workspace;
         var oldXY = oldBlock.getRelativeToSurfaceXY();
         var oldDisabled = oldBlock.isDisabledInStack();
         ...
         var codeFunc = Blockly.Python[oldBlock.type];
         if (typeof codeFunc != 'function') {
           throw Error ("Could not generate code from", oldBlock.type);
         }
         var code = codeFunc.call(oldBlock, oldBlock);
         code = code.replace(/\n/g, "");
 
         // Step 2: the block's existing connections
         var previousConnection = null;
         if (oldBlock.previousConnection && oldBlock.previousConnection.isConnected()) {
           previousConnection = oldBlock.previousConnection.targetConnection;
         }
 
         var nextConnection = null;
         if (oldBlock.nextConnection && oldBlock.nextConnection.isConnected()) {
           nextConnection = oldBlock.nextConnection.targetConnection;
         }
 
         Blockly.Events.setGroup(true);
 
         // Create temporary blocks to generate the XML structure of the new mixed-mode block
         // We don't track these events as they we don't need to
         Blockly.Events.disable();
         var tempBlock = null;
         
         if (oldBlock.type === "procedures_call") {
           tempBlock = ws.newBlock("mixed_singleline");
         } else {
           tempBlock = ws.newBlock(oldBlock.type.split("_")[0] + "_mixed_singleline");
         }
         
         var tempBlockShadow = ws.newBlock("mixed_singleline_text");
         ...
         var tempBlockInput = tempBlock.getInput("SINGLELINE_INPUT");
         tempBlockInput.connection.connect(tempBlockShadow.outputConnection);
         tempBlockInput.connection.setShadowDom(Blockly.Xml.blockToDom(tempBlockShadow));        
 
         var newBlockXml = Blockly.Xml.blockToDom(tempBlock);
 
         // Clean up temporary blocks and re-enable events
         tempBlockShadow.dispose();
         tempBlock.dispose();
         Blockly.Events.enable();
 
         var newBlock = Blockly.Xml.domToBlock(newBlockXml, ws);
         ...
         if (nextConnection) {
           newBlock.nextConnection.connect(nextConnection.getSourceBlock().previousConnection);
         }
         ...
         if (previousConnection || nextConnection) {
           ...
           newBlock.moveBy(oldXY.x - newXY.x, oldXY.y - newXY.y);
         }
 
         // Make sure the new block is also disabled if the old one was
         if (oldBlock.isDisabledInStack()) {
           newBlock.setDisabled(true);
           newBlock.updateBlockTreeStateColours(newBlock.isDisabledInStack());
         }
 
         Blockly.Events.setGroup(false);
       } catch (err) {
         console.error("could not convert", oldBlock.type, "due to:", err);
       }
     }, 0);
   }
 }
