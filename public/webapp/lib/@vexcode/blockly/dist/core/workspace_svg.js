...
Blockly.WorkspaceSvg.prototype.addMonitorIcon = function() {
  ...
  // Re-render to show the monitor area
  this.monitorIcon.setStateShowMonitor(true);
  this.monitorIcon.setState(false, false);
};
// RM Code End
 ...
 * @package
 */
Blockly.WorkspaceSvg.prototype.hideMonitorIcon = function() {
  if (!this.monitorIcon) {
    throw Error('Missing require for Blockly.MonitorIcon');
  }

  this.monitorIcon.setStateShowMonitor(false);
};
// RM Code End
 ...
 * @package
 */
Blockly.WorkspaceSvg.prototype.showMonitorIcon = function() {
  if (!this.monitorIcon) {
    throw Error('Missing require for Blockly.MonitorIcon');
  }
...
Blockly.WorkspaceSvg.prototype.isMonitorArea = function(e) {
  if ( this.monitorIcon && this.monitorIcon.getShowMonitor() && this.monitorAreaIcon_ &&
...
Blockly.WorkspaceSvg.prototype.centerOnBlock = function(id) {
  ...
  var bubbleWidth = 0;
  if (block.warning) {
    bubbleWidth = block.warning.getBubbleWidth();
  }
  // RM Code End
  ...
  var blockCenterX = xy.x + (multiplier * ((heightWidth.width / 2) - bubbleWidth));
...
Blockly.WorkspaceSvg.prototype.moveWorkspaceTopLeft = function(id) {
  ...
  var xy = block.getRelativeToSurfaceXY();
  // Height/width is in workspace units.
  // The centerOnBlock function grabs the height of the whole stack,
  // placing the stack in the center instead of the passed in block
...
Blockly.WorkspaceSvg.prototype.refreshToolbox = function() {
  var tmpToolbox = this.getToolbox();
  var tmpFlyout = tmpToolbox.getFlyout();
  var hideAfterRefresh = tmpFlyout.autoClose && !tmpFlyout.isVisible();
  this.refreshToolboxSelection();
  if (hideAfterRefresh) {
    tmpToolbox.clearSelection();
  }
};
