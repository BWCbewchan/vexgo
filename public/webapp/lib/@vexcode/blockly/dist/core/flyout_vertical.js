 ...
 * @type {number}
 */
Blockly.VerticalFlyout.maxToolboxWidth = 0;
// RM Code End
...
Blockly.VerticalFlyout.prototype.reflowInternal_ = function() {
  ...
  // RM Code Start
  // Check to see if the toolbox would be more than the max width.
  // If the max width is 0, there is no max width.
  var maxWidth = Blockly.VerticalFlyout.maxToolboxWidth;
  if (maxWidth && flyoutWidth > maxWidth) {
    // There is a max width that has been set and the toolbox would
    // be wider that the width. so wee need to set the width to the
    // max limit. This does not impact the actual workspace that shows
    // the blocks.
    flyoutWidth = maxWidth;
  }
  // RM Code End
