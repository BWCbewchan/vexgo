...
// RM Code Start
goog.require('Blockly.blockRendering.RoundCornerLarge');
// RM Code End
...
Blockly.blockRendering.RenderInfo.prototype.populateTopRow_ = function() {
  ...
  // RM Code Start
  } else if (this.block_.hat === 'define') {
    this.topRow.elements.push(
        new Blockly.blockRendering.RoundCornerLarge(this.constants_));
  // RM Code End
  } else {
    this.topRow.elements.push(
  ...
  // RM Code Start
    ...
    this.topRow.elements.push(
        new Blockly.blockRendering.RoundCornerLarge(this.constants_, 'right'));
  // RM Code End
  } else {
    this.topRow.elements.push(
...
Blockly.blockRendering.RenderInfo.prototype.addInput_ = function(input, activeRow) {
  ...
  if (this.isInline && input.type == Blockly.INPUT_VALUE) {
    ...
    // RM Code Start
    // we need to make sure we have the min hight for the notch hight offest change
    activeRow.minHeight = Math.max(activeRow.minHeight, activeRow.elements[activeRow.elements.length - 1].height);
    // RM Code End
