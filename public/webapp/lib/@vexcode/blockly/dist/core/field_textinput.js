...
Blockly.FieldTextInput.prototype.showPromptEditor_ = function() {
  var fieldText = this;
  // RM Code Start
  // make validator array
  var validators = [];
  validators.push(this.doClassValidation_.bind(this));
  if (this.validator_) {
    validators.push(this.validator_.bind(this));
  }
  // RM Code End
 ...
 * @protected
 */
Blockly.FieldTextInput._IPAD_FOCUS_FIX_NEEDED = true;
// RM Code End
...
Blockly.FieldTextInput.prototype.showInlineEditor_ = function(quietInput) {
  ...
  if (!quietInput) {
    // RM Code Start
    this.htmlInput_.focus({preventScroll: !Blockly.utils.userAgent.IPAD});
    // RM Code End
    this.htmlInput_.select();

    // RM Code Start
    // for iPad, we want to make sure that the inputs are not hidden behind the
    // onscreen keyboard. To do this we have a bit of a hack. the very first time
    // that a user clicks into a text field, we blur that input. At this point all
    // future input selections will move to ensure that they are not hidden by the
    // keyboard. This is not ideal, but it is better that not seeing the input or
    // not being able to edit any inputs due to the keyboard never staying open.
    if (Blockly.utils.userAgent.IPAD && Blockly.FieldTextInput._IPAD_FOCUS_FIX_NEEDED) {
      Blockly.FieldTextInput._IPAD_FOCUS_FIX_NEEDED = false;
